<?php
    ...
    $Q = "SELECT * FROM oc_t_user ORDER BY id ASC LIMIT 10";
    		...
    		logEvent('user', 'created new user - old_id:'.$current_user_id.', new_id:'.$new_user_id);    
    			...
    			logEvent('post', 'created new post - for user :'.$new_user_id.', new_post_id:'.$new_post_id);    
                            ...
                            if (!is_null($postRow['s_name'])){
                                ...
                                echo $new_post_image_Q = "INSERT INTO picture_post (picture_id, post_id, featured, image_path) VALUES (NULL, $new_post_id, '0', '".$postRow['s_path'].$postRow['s_name'].'.'.$postRow['s_extension']."');";
    ...
    function getUserIdFromNewDb($mobile_number){
        ...
        $host = "localhost"; $user = "rajesh_stage"; $password = "!0dulk@R";
        $tengai_db = @mysqli_connect($host, $user, $password, 'tengai_qa_pre_production') OR die ("Could not connect to MySQL: ".  mysqli_connect_error());
