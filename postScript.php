                ...
                while ($postRow = mysqli_fetch_assoc($p_result)){
                    ...
                    echo $new_post_Q = "
						INSERT INTO posts (title, description, category_id, user_id, country_id, city_id, suburb_id, post_address, 
						    ...
						    sf_entity_id, in_stock, store_customer_id, alternative_contacts, subcategory_id, slug) 
						VALUES ('".$postRow['s_title']."','".$postRow['s_description']."','".getCategoryId($postRow['fk_i_category_id'])."', '$new_user_id', 56,'".getCityId($postRow['s_region'])."','".getSuburbId($postRow['s_city'])."', '".$postRow['s_address']."', '".getSuburbId($postRow['i_price'])."', '".$postRow['fk_moderator_action']."', NULL, '".$postRow['dt_creat_date']."', 'classified', '0', '1', 1, NULL, '".$postRow['s_contact_additional']."', 1,'".$slug."')";

                    $host = "localhost"; $user = "root"; $password = "10dulk@R";
                    $tengai_db = @mysqli_connect($host, $user, $password, 'tengai_qa_pre_production') OR die ("Could not connect to MySQL: ".  mysqli_connect_error());
...
function getCategoryId($category_id){
    ...
    $host = "localhost"; $user = "root"; $password = "10dulk@R";
    $ownai_db = @mysqli_connect($host, $user, $password, 'ownai_live') OR die ("Could not connect to MySQL: ".  mysqli_connect_error());
...
function userExistsInNewDb($fuid){

    $host = "localhost"; $user = "root"; $password = "10dulk@R";
    $tengai_db = @mysqli_connect($host, $user, $password, 'tengai_qa_pre_production') OR die ("Could not connect to MySQL: ".  mysqli_connect_error());

    $query_result = "";
    $query = "SELECT company_name FROM user WHERE fuid = '$fuid'";
    $result = mysqli_query($dbc, $query); $resRow = mysqli_fetch_assoc($result);
    $query_result = $resRow['company_name'];
...
function getUserIdFromNewDb($mobile_number){
    ...
    $host = "localhost"; $user = "root"; $password = "!0dulk@R";
    $tengai_db = @mysqli_connect($host, $user, $password, 'tengai_qa_pre_production') OR die ("Could not connect to MySQL: ".  mysqli_connect_error());
